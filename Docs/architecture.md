...
Sources/SiriusMarkdownCore/
  ...
  RichContent/MarkdownHTMLSemanticAdapter.swift — sanitized HTML5 tree → native model
  RichContent/DefaultMarkdownLinkMetadataResolver.swift — bounded public favicon discovery
  Model/MarkdownModel.swift               — MarkdownSnapshot, MarkdownBlock, IDs, ranges
  Model/MarkdownLinkMetadata.swift        — shared link metadata/decorations/resource policy
...
Sources/SiriusMarkdownSwiftUI/
  ...
  Views/MarkdownStreamingRegionLayout.swift — bounded live-stream regions and cached region sizing
...
- **Rich content**: `MarkdownHTMLSemanticAdapter` uses SwiftSoup's HTML5 tree recovery to sanitize and normalize the supported HTML subset into `MarkdownRichContent` and ordinary source-mapped native blocks/runs. It drops active subtrees, unwraps safe unsupported containers, and keeps links and images under the existing policy paths. `DefaultMarkdownLinkMetadataResolver` owns optional public-origin icon discovery, validation, caching, and network isolation; no host favicon bundle is assumed.
- **Model**: `MarkdownSnapshot` exposes `blocks`, ordered `items` (blocks interleaved with `MarkdownHostBoundary`), `generation`, `sourceLength`, `isFinished`. `MarkdownRichContent`, `MarkdownLinkMetadata`, and atomic `MarkdownLinkDecoration` values cross into preparation without retaining parser nodes. Identity for rendering uses `MarkdownBlock.id`, not array position.
- **Inline layout**: `PreparedInlineContent`, `VariableWidthLineWalker`, `InlineLayoutEngine` with bounded caches for prepared, measured, and laid-out inline results; `CoreTextInlineMeasurer` where CoreText is available. A bounded shared `MarkdownCoreTextMeasurementCache` reuses fonts and unchanged token widths across tail preparation values with full font/presentation identity. Link-decoration segments carry a prepared keep-with-next relationship through the first label token, so width-only layout cannot orphan a symbol/favicon from its label. This is the Pretext-style **prepare** vs **layout** split for measurements that must not live in SwiftUI `body`.
- **Policy**: protocols (`MarkdownLinkPolicy`, `MarkdownImagePolicy`, `MarkdownHTMLPolicy`, `MarkdownRemoteResourcePolicy`, etc.) and `DefaultMarkdownPolicy`. Safe link schemes are allowed, content-image loading remains denied by default, and the documented HTML subset is authorized only because conversion sanitizes it into inert native semantics. Remote icon discovery has a separate HTTPS/public-network policy.
- **Diagnostics**: `MarkdownDiagnosticsRecorder` / `MarkdownDiagnosticsCounters` (parse, tail vs sealed parse, boundary scan work, render preparation, highlighting calls and bytes, math, prepare, layout, width relayout, cache hits/misses) plus signpost helpers.

### SwiftUI

- **`MarkdownDocumentView`** (default theme `.document`) and **`StreamingMarkdownView`** (default `.compactChat`) should be driven with precomputed `MarkdownPreparedSnapshot` values. Direct `snapshot:` initializers remain as deprecated compatibility shims only; they enforce cheap code/math/HTML safety policy decisions but skip full highlighting/rendering/layout preparation, so long or streaming content should prepare in the host model layer. The streaming view mounts items in stable regions of at most 16 and caches sealed region sizes by revision and proposal width; only the final mutable region normally remeasures after an append. Dynamic prepared inline leaves publish a compact settled-layout generation inside those regions, so a deferred width reflow invalidates provisional region geometry without changing leaf identity. Fixed-width table/list leaves consume their latest immutable `initialLayoutResult` directly rather than mirroring its byte ranges in retained SwiftUI state.
- **`MarkdownRendererConfiguration`** wires `MarkdownTheme`, policies, optional `MarkdownLinkAction`, `MarkdownCopyProvider`, `MarkdownLinkMetadataResolver`, `MarkdownLinkDecorationConfiguration`, `MarkdownCodeHighlighter`, and `MarkdownMathRenderer` (`DefaultMarkdownLinkMetadataResolver`, `DefaultMarkdownCodeHighlighter`, and `PlainMarkdownMathRenderer` ship as defaults; each remote-link/decoration surface is replaceable or disableable). Its `prepare(block:)` / `prepare(snapshot:)` methods move inline attributed text, measured inline content, link/image policy decisions, code highlighting, math rendering, sanitized HTML, and cached link decoration selection out of block `body` evaluation. Automatic link decoration prepares a font-relative template SF Symbol in the same attachment box later used by a branded favicon; custom glyph/fixed-size configuration remains available. Runtime icon resolution is scheduled by `MarkdownRenderSession`, never by a SwiftUI body, and republishes through the same prepared diff path. CoreText line-plan reuse keys the complete prepared-content fingerprint as well as geometry. `MarkdownRenderPreparationCache` bounds reuse for sealed values; mutable values bypass stable caches. Plain and Highlight.js-backed languages use rolling suffix work with real parser continuation, 16 KiB full-context checkpoints, and a full highlight on seal; native Swift and arbitrary custom highlighters keep full-document semantics. The JavaScriptCore bridge roots retained functions and heap-stored arguments/results with `JSValueProtect` for their complete native lifetime; forced-GC tests enforce that boundary for Highlight.js and Mermaid.
...
- **`MarkdownBlockView`** branches on `MarkdownBlockKind` for structured blocks and consumes `MarkdownPreparedBlockContent` for inline text, lists, nested lists, tables, code, math, and sanitized native rich HTML. Rich HTML recursively uses already-prepared ordinary block families, not raw tag text or WebKit. List and table rendering uses prepared source-range IDs rather than array offsets. A mutable table retains its completed prepared row prefix, reuses unchanged cell inline layouts, incrementally updates column maxima, and publishes bounded streaming width revisions. SwiftUI consumes those widths directly through a persistent row-measurement cache; the view layer does not reparse table Markdown/HTML, scan historical cells, or measure raw source.
...
- **Selection feel — drag affinity and contexts** (`MarkdownDocumentSelectionGeometry.swift`, `MarkdownInteraction.swift`, `MarkdownDocumentView.swift`):
  ...
  - *macOS document interaction:* One bounded AppKit event handler owns document mouse and keyboard selection by default, using the existing prepared source geometry across block boundaries. Native selectable text leaves remain an explicit compatibility mode and are excluded from document event handling. Link menus use native `NSMenu` actions for the actual prepared destination. Math-containing paragraphs retain baseline-aligned TextKit attachments while the document handler owns selection.
...
- stable streaming region revisions must survive mutable-tail publications, genuine AppKit-hosted rapid publication must avoid synchronous constraint recursion, and mounted wide-to-narrow reflow must increase the region's natural height before later sibling surfaces begin;
- growing Highlight.js-backed code highlighting must process suffix bytes between checkpoints, preserve multiline lexical context, and fully rehighlight on seal, while native Swift and custom highlighters retain full-document semantics;
- equivalent CoreText measurements must reuse the shared bounded width/font cache across preparation values;
