...
> Product status: the packaged chat and document presets use proposal-contained prepared-line rendering, default-on source-backed document selection, reference-style links resolve through `swift-markdown` even across sealed streaming regions, the default code highlighter is language-aware for explicit supported fences where JavaScriptCore/highlight.js is available, Mermaid fences prepare concrete-color light/dark SVG output and geometry off the SwiftUI hot path, Mermaid diagrams render in a bounded package-owned pan/zoom viewport, document/code affordances are generic and source-backed with explicit accessibility labels, and the release gate includes strict required-group Swift-vs-Pretext fixture comparison, AppKit render probes for document, document affordance chrome, compact chat, transcript wrapping, multilingual, inline-attribute, overflow, hard-break, long-word, finite-column containment, wide-to-narrow resize, Mermaid diagram pan/zoom, code-highlighting output, required test discovery, and a clean local SwiftPM consumer build, plus AppKit-hosted transcript command clipping regressions.
...
SiriusMarkdown's default tests assert this contract through renderer preparation, diagnostics, AppKit `MarkdownDocumentView` and `MarkdownDocumentSurface` render probes, exact source-backed selection copy tests, CRLF streaming equivalence, protocol-relative link denial, single-call math/HTML render-plan policy checks, and strict Pretext comparison: large streaming transcripts must keep stable prepared item IDs, repeated preparation must hit inline/code/math caches, width changes must reuse measured inline content, document collapse must preserve prepared identity, and Pretext drift must fail instead of being hidden as a passing known issue.
