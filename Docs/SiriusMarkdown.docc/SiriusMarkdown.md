...
Authorized HTML is parsed through a standards-aware HTML5 tree, sanitized, and converted into the same source-mapped native blocks and inline runs as Markdown. Active scripts, embedded browsing/plugin content, active controls, and unsafe resources are dropped or governed by the ordinary link/image policies; no WebKit document or CSS cascade is introduced. Markdown links and HTML anchors share one decoration path. Every policy-authorized destination that normalizes into an activatable URL receives an immediate native glyph, while `MarkdownRenderSession` limits asynchronous discovery through the replaceable `DefaultMarkdownLinkMetadataResolver` to public HTTPS site icons. Resolved icons replace that glyph only after bounded native decoding confirms meaningful visible pixel coverage; transparent or effectively invisible files remain on the glyph. Set `linkMetadataResolver` to `nil` or `linkDecoration` to `.disabled` to opt out.
