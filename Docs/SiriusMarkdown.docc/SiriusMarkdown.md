...
In production paths, prefer **`MarkdownRenderSession`** or keep **`MarkdownRendererConfiguration`** alive in your model layer and call **`prepare(snapshot:)`** before SwiftUI evaluates renderer bodies. Prepared snapshots carry policy-gated inline text, measured inline content, image decisions, highlighted code, rendered math, sanitized native HTML, link decorations, table cells, list items, selection/copy source ranges, and host-boundary ordering. The compatibility `snapshot:` view initializers are deprecated because they skip full preparation and are only suitable for small compatibility surfaces; they still enforce cheap code, math, and HTML policy decisions.
...
Authorized HTML is parsed through a standards-aware HTML5 tree, sanitized, and converted into the same source-mapped native blocks and inline runs as Markdown. Active scripts, embedded browsing/plugin content, active controls, and unsafe resources are dropped or governed by the ordinary link/image policies; no WebKit document or CSS cascade is introduced. Markdown links and HTML anchors share one decoration path. Every policy-authorized destination that normalizes into an activatable URL receives an immediate template SF Symbol in `.automatic` mode, sized to the surrounding text up to the configured maximum. `MarkdownRenderSession` limits asynchronous discovery through the replaceable `DefaultMarkdownLinkMetadataResolver` to public HTTPS site icons. Resolved icons replace that symbol in the same reserved attachment box only after bounded native decoding confirms meaningful visible pixel coverage; transparent or effectively invisible files remain on the symbol. Decoration, nonbreaking spacer, and the first label token stay in one wrapping unit. Set `linkMetadataResolver` to `nil` or `linkDecoration` to `.disabled` to opt out; construct `MarkdownLinkDecorationConfiguration` directly to retain a custom text glyph or fixed icon size.

Allowed content images (an explicit `MarkdownImagePolicy` `.allow` decision) flow as prepared inline attachments instead of alt text. `MarkdownPreparedAttachment` carries reserved `pointWidth`/`pointHeight`/`ascent`/`descent` box metrics — from a cheap ImageIO header probe of resolver-supplied bytes when available, otherwise `MarkdownTheme.attachmentPlaceholder`'s default box — and the CoreText line plan reserves a gap for that box via a `CTRunDelegate` instead of measuring alt text. One AppKit/UIKit host view exists per attachment instance, drawing either the resolved bytes or quiet reserved-box chrome; width-only relayout never re-invokes the resolver. Denied images (the default) are unaffected and keep rendering alt/`[image: reason]` text. This is the placement layer only: SiriusMarkdown does not fetch content images over the network or decode/animate multi-frame formats on its own — those are separate, independently opt-in capabilities that mount inside these same attachment slots. Package-owned favicon resolution is the narrow, separately governed link-decoration path described above.

`MarkdownDocumentSurface` is the package-owned document chrome for static reader surfaces. It wraps already-prepared content with optional copy, export/download, and collapse controls. On macOS, `MarkdownRendererConfiguration` defaults to `.enabled` native text selection and `.disabled` source-backed document selection: bounded noneditable `NSTextView` leaves provide system selection color, word/drag/keyboard selection, continuous-prose copy, and the standard AppKit contextual menu. Image-backed inline math remains inside that selectable TextKit leaf as a prepared attachment; streamed fallback/image transitions preserve semantic selection ranges. Mermaid ASCII and invalid-image math text fallbacks use the same leaf. Per-block SwiftUI context menus do not intercept secondary clicks. Hosts that require exact-source cross-block selection can set `documentSelection = .enabled`; the configuration then disables native leaf selection so the two gesture owners never compete. Other platforms retain the source-backed document-selection default. `MarkdownCopyProvider` can expose exact full-document Markdown in addition to range slices, and `MarkdownAffordanceActionHandler` lets host apps replace pasteboard/export behavior without changing the renderer model or moving work into view bodies.
...
Heading typography is configured through **`MarkdownTheme.headings`** for H1 through H6, similar to CSS `h1`...`h6` styling. Each **`MarkdownTextStyle`** carries the SwiftUI **`Font`** used for visible rendering plus the `fontSize`, `lineHeight`, and **`MarkdownInlineFontProfiles`** used by CoreText measurement during render preparation. A requested line height remains unchanged when it fits those profiles and otherwise uses their cached typographic minimum so ascenders and descenders are not clipped. Do not rely on SiriusMarkdown to infer measurement profiles from arbitrary SwiftUI fonts; pass matching profiles when using custom fonts.
...
`MarkdownRendererConfiguration.compactChat`, `.document`, and direct custom configuration retain `MarkdownInlineRenderingMode.coreTextPaintedLines` as the prepared rendering mode. On macOS, the default native-selection branch presents the continuous attributed content through bounded AppKit text views instead of inserting prepared visual-wrap newlines or painting the custom selection overlay. Explicit source-backed document selection uses the CoreText-painted prepared-line surface. `MarkdownInlineRenderingMode.preparedNativeLines` and `.systemText` remain explicit compatibility fallbacks.
