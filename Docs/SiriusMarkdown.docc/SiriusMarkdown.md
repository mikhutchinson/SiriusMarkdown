...
Authorized HTML is parsed through a standards-aware HTML5 tree, sanitized, and converted into the same source-mapped native blocks and inline runs as Markdown. Active scripts, embedded browsing/plugin content, active controls, and unsafe resources are dropped or governed by the ordinary link/image policies; no WebKit document or CSS cascade is introduced. Markdown website links and HTML anchors share one decoration path. They receive an immediate native glyph, and `MarkdownRenderSession` can use the replaceable `DefaultMarkdownLinkMetadataResolver` to discover and validate public HTTPS site icons asynchronously. Set `linkMetadataResolver` to `nil` or `linkDecoration` to `.disabled` to opt out.
