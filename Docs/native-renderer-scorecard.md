...
- Interaction: document selection is the default, with a single bounded AppKit event handler on macOS and prepared source-backed geometry. Native leaf selection remains available explicitly. Painted and native link surfaces provide Open Link and Copy Link Address context menus; math-containing paragraphs retain native attachments. The source-backed selector writes plain text and exact Markdown on Cmd-C without network or WebKit work. Product quality is established by mounted event and rendering tests, not by a comparative best-in-class claim.
