...
- Inline rendering: packaged chat and document presets plus direct custom configurations use `MarkdownInlineRenderingMode.coreTextPaintedLines` by default, so visible wrapping is driven by cached prepared line ranges and whole-line CoreText painting contained to the finite proposal. `MarkdownInlineRenderingMode.preparedNativeLines` and `.systemText` remain explicit compatibility fallbacks.
