...
This runbook is the local release authority for `SiriusMarkdown`. For the current public package release, use `0.5.8` as the tag and do not publish unless every release blocker below is clear.
...
The release-gate discovery floor for this slice is `511` Swift tests.
...
5. Commit the release candidate:
   ...
   git commit -m "Prepare SiriusMarkdown 0.5.8 release"
   ```

6. Tag and push:

   ```sh
   git tag -a 0.5.8 -m "SiriusMarkdown 0.5.8"
