...
This checkout is the `0.3.1` public package release. The release gate is strict Swift-vs-Pretext layout comparison, required Pretext product fixture groups, and AppKit render probes for document, compact chat, multilingual, inline-attribute, overflow, hard-break, long-word, finite-column containment, and language-aware code highlighting output. Fixture drift, missing groups, duplicate fixture names/groups, and trivial render output are release blockers.

The `0.3.1` product claim is native SwiftUI Markdown rendering with prepared-line layout, streaming snapshots, bounded caches, safe default policies, language-aware default code highlighting, code-block language/copy affordances, public chat/document presets, and first-class H1-H6 heading typography through `MarkdownTheme.headings`. It is not a custom glyph renderer: `preparedNativeLines` slices prepared attributed line ranges and renders them with SwiftUI `Text(AttributedString)`.
...
```swift
dependencies: [
    .package(url: "https://github.com/mikhutchinson/SiriusMarkdown.git", from: "0.3.1")
],
targets: [
    .target(
        name: "YourApp",
...
`0.3.1` is ready to publish only when:
...
- the release commit is tagged as `0.3.1` and pushed with tags.
