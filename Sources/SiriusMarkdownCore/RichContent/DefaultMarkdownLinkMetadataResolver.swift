    ...
    @unchecked Sendable
{
    ...
    private typealias HTTPPayload = MarkdownRemoteHTTPPayload
    ...
    public init(
        ...
        self.policy = policy
        self.transport = MarkdownRemoteResourceTransport(policy: policy, limits: limits, sessionConfiguration: sessionConfiguration)
        self.limits = limits
    ...
    private func fetch(
        ...
        try await transport.fetch(initialURL, kind: kind, maximumBytes: maximumBytes, acceptedMIMETypes: acceptedMIMETypes, deadline: deadline)
    }
...
enum MarkdownLinkMetadataError: Error, CustomStringConvertible {
